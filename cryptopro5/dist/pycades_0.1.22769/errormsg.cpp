...
* \author $Author: cross $
*
* \brief –еализаци€ форматировани€ сообщений об ошибках.
*
* –еализаци€ форматировани€ сообщений об ошибках.
*/
...
#   define STRICT // ƒл€ более строгой типизации. Ќе верить за€влени€м MS, что
          // в Visual C++ STRICT определено по умолчанию.
#   define WIN32_LEAN_AND_MEAN // ƒл€ ускорени€ компил€ции
#   define _WIN32_WINNT 0x0501 // ƒолжно работать начина€ с XP
...
static LPCWSTR rus_wrong_name = L"Ќеверный формат имени или попытка открыть контейнер другого криптопровайдера.";
    ...
    HRESULT hr, LPTSTR szBuf, DWORD chBufLen, DWORD dwLangId)
{
    ...
    case FACILITY_TSPCLI:
        {
            ...
            if (hInst)
            {
                //TODO: ≈сли конструктор выбросит исключение, то здесь случитс€
                //полна€ фигн€.
    ...
    {
        // ƒл€ наших кодов ошибок попробуем найти сообщение в модуле,
        // содержащем данный исполн€емый код. Ќапример, cadescom.dll
        // получит сообщение именно таким путЄм.
        //
        // ¬ качестве адреса в GetModuleHandleEx(
        // GET_MODULE_HANDLE_EX_FLAG_FROM_ADDRESS) передаЄм адрес литерала,
        // который гарантированно будет в адресном пространстве данного
        // модул€.  од вовзрата намеренно игнорируем, т.к. нам достаточно
        // того, что в hInst останетс€ 0.
    ...
    if (hInst) {
        ...
        // ≈сли не нашли сообщение в конкретно заданном €зыке,
        // спросим в €зыке 0 (будет поиск по разным системным €зыкам)
        if (!dwRet && dwLangId)
    ...
    if (hInst)
    {
        // “ут используем dwCode, а не полный hr!
...
const ATL::CAtlStringW GetErrorMessage(HRESULT hr, DWORD dwLangId){
    ...
    switch(hr){
        ...
        case 0x8007006E:
            ...
            break;
        case NTE_BAD_ALGID:
        case CRYPT_E_OID_FORMAT: // такой код может быть если передать oid алгоритма который мы не знаем на UNIX
